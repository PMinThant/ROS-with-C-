...
int main(int argc, char** argv)
{
    ...
    if(argc != 3)
    {
        ROS_WARN("Usage: rosrun action_pmt demo_client<goal> <time_to_preempt>");
        return -1;
    }
    actionlib::SimpleActionClient<action_pmt::demoAction> ac("demo_action", true);

    ROS_INFO("Waiting for action server ...");

    ac.waitForServer();
    action_pmt::demoGoal goal;
